<script type="text/javascript">
	...
	function writeAllStatus(current_status, all_data){
		  ...
		  headers: { 'Authorization': `Bearer ${authtk}` },
			...
			if (!response_count) {	
				...
				response_data.map(function(currentValue){
					writeAllStatus(currentValue, response)
				})
	...
	function writeRecentPortfolio(current_portfolio){
		let root = document.getElementById('recentPortfolio')
		$(root).slideDown()
		let writeContent = new Promise(function(resolve){
			let write = $(root).append(`			
				<li class="inline-items">
					<div class="author-thumb">
						<img src="${current_portfolio.pictures[0]}" alt="banner">
					</div>
					<div class="notification-event">
						<a href="#" class="h6 notification-friend">${current_portfolio.title}</a>
						<span class="chat-message-item">${current_portfolio.client}</span>
					</div>
					<!-- <span class="notification-icon" data-toggle="tooltip" data-placement="top" data-original-title="VIEW PORTFOLIO">
						<a href="#">
	...
	function loadRecentPortfolio(){
		...
		let targetUrl = `${devUrl}/api/user/${_id}/portfolio?page=1&limit=2`;
		$(root).hide()
		...
		$.ajax({
		  ...
		  contentType : false,
			...
			response_data.map(function(currentValue){
				writeRecentPortfolio(currentValue)
			})
	...
	function writeRecentBids(current_bid){
		let root = document.getElementById('recentBids')
		$(root).slideDown()
		...
		let writeContent = new Promise(function(resolve){
			let write = $(root).append(`			
				<li class="inline-items">
					<div class="author-thumb">
						<img src="${current_bid.pictures[0]}" alt="banner">
					</div>
					<div class="notification-event">
						<a href="#" class="h6 notification-friend">${current_bid.title}</a>
						<span class="chat-message-item">${current_bid.location}</span>
					</div>
					<!-- <span class="notification-icon" data-toggle="tooltip" data-placement="top" data-original-title="VIEW BID">
						<a href="#">
							<svg class="olymp-magnifying-glass-icon"><use xlink:href="#olymp-magnifying-glass-icon"></use></svg>
						</a>
					</span> -->
	...
	function loadRecentBids(){
		...
		let targetUrl = `${devUrl}/api/user/${_id}/bid?page=1&limit=2`;
		$(root).hide()
	...
	function writeProfile(){
		let statuses = new Promise(function(resolve,error){
			loadStatus(1);
			resolve('fetched')
		})

		statuses.then(function(){
			let recent_portfolio = new Promise(function(resolve,error){
				loadRecentPortfolio()
				resolve('fetched')
			})			
			recent_portfolio.then(function(){
				let recent_bids = new Promise(function(resolve,error){
					loadRecentBids()
					resolve('fetched')
				})
			})
